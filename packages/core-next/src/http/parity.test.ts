...
import { SseHttpService } from "../../../core/src/http/services/sse-http-service.ts";
import { HttpResponseFactory as OldResponses } from "../../../core/src/http/responses.ts";
import { SseEventEncoder as OldSseEvents } from "../../../core/src/http/sse-event-encoder.ts";
import { MessageBodyCodec as OldBodyCodec } from "../../../core/src/http/message-body-codec.ts";
import { Streams, StreamsReader, ZERO_OFFSET } from "../index.ts";
  ...
  it(`old/new expiry parsing and HEAD: ${fixture.name}`, async () => {
    const old = createHttpHandler({
      protocol: new StreamProtocol({
        ...
        storage: { adapter: { ...createMemoryStorageAdapter(), scheduleExpiry: () => {} } },
        clock: { now: () => fixedTime, date: (value) => new Date(value ?? fixedTime) },
      }),
    });
    ...
    try {
      for (const method of ["PUT", "HEAD"] as const) {
        ...
        expect(right.statusText).toBe(left.statusText);
        expect(Object.fromEntries(right.headers)).toEqual(Object.fromEntries(left.headers));
        // The old pure handler returns error text for HEAD; its Bun wire and the new edge omit it.
        const oldWire =
          method === "HEAD"
            ? new Response(null, { status: left.status, headers: left.headers })
            : left;
        expect(new Uint8Array(await right.arrayBuffer())).toEqual(
          new Uint8Array(await oldWire.arrayBuffer()),
        );
        if (method === "HEAD" && fixture.head === 200) {
          const expected = fixture.headers["stream-ttl"] ?? fixture.headers["stream-expires-at"];
          if (expected === undefined) throw new Error("Missing fixture expiry header");
          expect(
...
// Compare complete event bytes, independent of transport chunk boundaries, then cancel both streams.
  ...
  it(`open SSE initial and real timeout frames match for ${contentType}`, async () => {
    ...
    const protocol = new StreamProtocol({
      ...
      clock: {
        ...
        date: (value) => new Date(value ?? fixedTime + 20_000),
      },
    });
    await protocol.create("open", { contentType, initialData: data });
    const bound = await protocol.get("open");
    if (bound.status !== "ok") throw new Error("Expected old stream");
    const oldResponses = new OldResponses();
    const oldSse = new SseHttpService({
      responses: oldResponses,
      sseEvents: new OldSseEvents(new OldBodyCodec()),
      clock: { now: () => fixedTime, date: (value) => new Date(value ?? fixedTime) },
    });
    ...
    try {
      ...
      left = oldResponses.secure(await oldSse.execute(bound.stream, "-1"));
      right = await edge.handler(new Request("http://example.test/open?offset=-1&live=sse"));
      expect(right.status).toBe(left.status);
      expect(right.statusText).toBe(left.statusText);
      expect(Object.fromEntries(right.headers)).toEqual(Object.fromEntries(left.headers));
      const [oldBytes, newBytes] = await Promise.all([firstEvents(left, 3), firstEvents(right, 3)]);
      ...
      if (left?.body && !left.body.locked) await left.body.cancel();
