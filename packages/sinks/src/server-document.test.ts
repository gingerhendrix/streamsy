...
): Promise<Response> => Effect.runPromise(handleDocumentSink(sink, request, { document }));
...
describe("handleDocumentSink", () => {
  test("serves the canonical document with the declared cache policy", async () => {
    const response = await serve(get(), () => Effect.succeed(summary(2)));
    expect(response.status).toBe(200);
    expect(await response.text()).toBe('{"issues":2,"workspaceId":"main"}');
  ...
  test("the entity tag is stable for an unchanged document and moves when it changes", async () => {
    const first = await serve(get(), () => Effect.succeed(summary(2)));
    const again = await serve(get(), () => Effect.succeed({ workspaceId: "main", issues: 2 }));
    const changed = await serve(get(), () => Effect.succeed(summary(3)));
    expect(again.headers.get("etag")).toBe(first.headers.get("etag"));
    expect(changed.headers.get("etag")).not.toBe(first.headers.get("etag"));
  });

  test("a conditional request on the current tag is 304 with no body", async () => {
    const first = await serve(get(), () => Effect.succeed(summary(2)));
    const etag = first.headers.get("etag") ?? "";
    const conditional = await serve(get({ "if-none-match": etag }), () =>
    ...
    const weak = await serve(get({ "if-none-match": `W/${etag}` }), () =>
      Effect.succeed(summary(2)),
    );
    expect(weak.status).toBe(304);

    const stale = await serve(get({ "if-none-match": etag }), () => Effect.succeed(summary(3)));
    expect(stale.status).toBe(200);
  });

  test("HEAD reports the validator without the body", async () => {
    const response = await serve(get({}, "HEAD"), () => Effect.succeed(summary(2)));
    expect(response.status).toBe(200);
    expect(await response.text()).toBe("");
    expect(response.headers.get("etag")).toMatch(/^"[0-9a-f]{16}"$/);
  });

  test("a document the declared schema rejects is a typed failure, never a cached wrong answer", async () => {
    const response = await serve(get(), () => Effect.succeed({ workspaceId: "main" }));
    expect(response.status).toBe(500);
    expect(await response.json()).toMatchObject({ _tag: "WireDecodeFailed" });
    expect(response.headers.get("cache-control")).toBe("no-store");
  });

  test("an unavailable document and a retired contract are typed and uncacheable", async () => {
    const unavailable = await serve(get(), () =>
      Effect.fail(new DocumentSinkSourceFailure({ detail: "store down" })),
    );
    expect(unavailable.status).toBe(503);
    expect(await unavailable.json()).toMatchObject({ _tag: "DocumentUnavailable" });

    const retired = await serve(get({ [DOCUMENT_SINK_CONTRACT_HEADER]: "retired" }), () =>
      Effect.succeed(summary(2)),
    );
  ...
  test("a path outside the checked route never reaches the capability", async () => {
    const response = await Effect.runPromise(
      handleDocumentSink(sink, new Request("http://host/document/main/other"), {
