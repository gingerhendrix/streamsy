...
import type { SourceAck } from "@streamsy/streams/causal";
import { AppendStreamsLive, ReadStreamsLive } from "@streamsy/streams";
import { DerivedRecoveryLive } from "./mesh/derived-append.ts";
import { canonicalLaneInput, deriveProducerLane } from "./mesh/lane.ts";
import { NonNegativeInt, NormalizedRequiredText, PositiveInt } from "./mesh/schemas.ts";
import {
...
class Client extends Context.Service<Client, ClientService>()(
  "@streamsy/projection/StateProjection/Client",
) {}
...
function publicLimit(limit: keyof import("./mesh/projection.ts").CatchUpLimits): keyof Limits {
  switch (limit) {
    case "maxPages":
      return "pages";
    case "maxBatches":
