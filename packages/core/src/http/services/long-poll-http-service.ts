...
export class LongPollHttpService {
  ...
  async execute(streamId: string, offset: string, cursor?: string): Promise<Response> {
    const result = await this.deps.protocol.readLive(streamId, { offset, mode: "long-poll", cursor });
  ...
  private toNoContentResponse(result: { nextOffset: string; cursor: string; closed?: boolean }): Response {
