...
`catchUp()` is a bounded, one-source framework operation, not a follow loop or a core processor interface. At startup it scans output State history to recover its authoritative lineage and producer sequence, so this incubation path is O(output history). It then resumes the source strictly after that domain-correct source position. Limits apply to complete source delivery boundaries: pages are delivered client batches, batches are committed State transactions, items are decoded source items, and bytes are the encoded source batch size. A boundary that would exceed a limit is left uncommitted for deterministic continuation. Filtered boundaries still commit a metadata-only State transaction. Cancellation after an accepted append reports durable progress explicitly; cancellation during an ambiguous append reports that durability is unknown and restart recovery must decide it.
