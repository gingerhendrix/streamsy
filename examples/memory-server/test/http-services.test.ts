...
import { AppendHttpService } from "../../../packages/core/src/http/services/append-http-service.ts";
import { CreateHttpService } from "../../../packages/core/src/http/services/create-http-service.ts";
import { DeleteHttpService } from "../../../packages/core/src/http/services/delete-http-service.ts";
import { LongPollHttpService } from "../../../packages/core/src/http/services/long-poll-http-service.ts";
import { MetadataHttpService } from "../../../packages/core/src/http/services/metadata-http-service.ts";
import { ReadHttpService } from "../../../packages/core/src/http/services/read-http-service.ts";
import { SseHttpService } from "../../../packages/core/src/http/services/sse-http-service.ts";
import { HttpDispatchService } from "../../../packages/core/src/http/dispatch-service.ts";
import { EtagBuilder } from "../../../packages/core/src/http/etag-builder.ts";
import { MessageBodyCodec } from "../../../packages/core/src/http/message-body-codec.ts";
import { ProducerHeaderParser } from "../../../packages/core/src/http/producer-header-parser.ts";
import { ReadQueryParser } from "../../../packages/core/src/http/read-query-parser.ts";
import { RequestBodyReader } from "../../../packages/core/src/http/request-body-reader.ts";
import { HttpResponseFactory } from "../../../packages/core/src/http/responses.ts";
import { SseEventEncoder } from "../../../packages/core/src/http/sse-event-encoder.ts";
import { StreamPathService } from "../../../packages/core/src/http/stream-path-service.ts";
import type { Clock, HttpRouteContext } from "../../../packages/core/src/http/types.ts";
...
function makeProtocolStub(): ProtocolStub {
  let createResult: CreateResult = { status: "created", nextOffset: "0_0", contentType: "application/octet-stream" };
  ...
  let readLiveResult: ReadLiveResult = { status: "timeout", messages: [], nextOffset: "0_0", upToDate: true, cursor: "c" };
  let metadataResult: MetadataResult = { status: "ok", contentType: "application/octet-stream", nextOffset: "0_0" };
...
function deps(stub: ProtocolStub) {
  ...
  const clock: Clock = { now: () => new Date("2026-01-01T00:00:00.000Z").getTime(), date: (value?: number | string) => new Date(value ?? "2026-01-01T00:00:00.000Z") };
  const sse = new SseHttpService({ protocol: stub.protocol, responses, sseEvents: new SseEventEncoder(bodyCodec), clock });
...
describe("CreateHttpService", () => {
  it("passes create options and normalizes empty JSON arrays as no initial data", async () => {
    ...
    stub.setCreate({ status: "created", nextOffset: "0_0", contentType: "application/json", closed: true });
    const responses = new HttpResponseFactory();
    ...
    const response = await service.execute(routeCtx(new Request("http://x/api/stream", {
      method: "PUT",
      body: "[]",
      headers: {
    ...
    expect(stub.createCalls).toEqual([{ streamId: "stream", options: { contentType: "application/json", ttlSeconds: 60, expiresAt: undefined, initialData: undefined, closed: true, forkedFrom: undefined, forkOffset: undefined } }]);
  });

  it("rejects invalid create headers and maps protocol conflicts", async () => {
    const stub = makeProtocolStub();
    const responses = new HttpResponseFactory();
    const service = new CreateHttpService({ protocol: stub.protocol, path: new StreamPathService("/"), responses, bodyReader: new RequestBodyReader(1024, responses) });
    expect((await service.execute(routeCtx(new Request("http://x/s", { method: "PUT", headers: { "stream-ttl": "bad" } })))).status).toBe(400);
    expect((await service.execute(routeCtx(new Request("http://x/s", { method: "PUT", headers: { "stream-fork-offset": "1_0" } })))).status).toBe(400);

    stub.setCreate({ status: "conflict", nextOffset: "0_0", contentType: "text/plain", errorMessage: "mismatch" });
...
describe("AppendHttpService", () => {
  it("validates body rules before calling protocol.append", async () => {
    const stub = makeProtocolStub();
    const responses = new HttpResponseFactory();
    const service = new AppendHttpService({ protocol: stub.protocol, responses, bodyReader: new RequestBodyReader(1024, responses), producerHeaders: new ProducerHeaderParser() });
    expect((await service.execute(routeCtx(new Request("http://x/s", { method: "POST", body: new Uint8Array([1, 2]) })))).status).toBe(400);
    expect((await service.execute(routeCtx(new Request("http://x/s", { method: "POST", body: "[]", headers: { "content-type": "application/json" } })))).status).toBe(400);
  ...
  it("passes producer/sequence options and maps appended/closed-conflict responses", async () => {
    ...
    const appended = await service.execute(routeCtx(new Request("http://x/s", {
      method: "POST",
      body: "hi",
      headers: { "content-type": "text/plain", "stream-seq": "seq-1", "producer-id": "p", "producer-epoch": "2", "producer-seq": "3" },
    })));
    expect(appended.status).toBe(200);
    expect(appended.headers.get("producer-epoch")).toBe("2");
    expect(stub.appendCalls[0]).toMatchObject({ streamId: "stream", options: { contentType: "text/plain", seq: "seq-1", producer: { producerId: "p", producerEpoch: 2, producerSeq: 3 } } });
    ...
    const closed = await service.execute(routeCtx(new Request("http://x/s", { method: "POST", body: "bye", headers: { "content-type": "text/plain" } })));
...
describe("ReadHttpService", () => {
  it("returns an offset=now tail response without reading messages", async () => {
    const stub = makeProtocolStub();
    stub.setMetadata({ status: "ok", contentType: "application/json", nextOffset: "2_0", closed: true });
    const { responses, bodyCodec, longPoll, sse } = deps(stub);
    const service = new ReadHttpService({ protocol: stub.protocol, responses, bodyCodec, readQuery: new ReadQueryParser(), etags: new EtagBuilder(), longPoll, sse });

    const response = await service.execute(routeCtx(new Request("http://x/s?offset=now", { method: "GET" })));
  ...
  it("builds catch-up bodies and honors If-None-Match", async () => {
    const stub = makeProtocolStub();
    stub.setRead({ status: "ok", messages: [message("1_0", "a"), message("2_0", "b")], nextOffset: "2_0", upToDate: true });
    stub.setMetadata({ status: "ok", contentType: "text/plain", nextOffset: "2_0" });
    const { responses, bodyCodec, longPoll, sse } = deps(stub);
    const service = new ReadHttpService({ protocol: stub.protocol, responses, bodyCodec, readQuery: new ReadQueryParser(), etags: new EtagBuilder(), longPoll, sse });

    const response = await service.execute(routeCtx(new Request("http://x/s?offset=-1", { method: "GET" })));
    expect(response.status).toBe(200);
    expect(response.headers.get("etag")).toBe(`"${btoa("/s")}:-1:2_0"`);
    expect(await response.text()).toBe("ab");

    const notModified = await service.execute(routeCtx(new Request("http://x/s?offset=-1", { method: "GET", headers: { "if-none-match": response.headers.get("etag")! } })));
...
describe("SseHttpService", () => {
  it("emits initial data and a closed control event", async () => {
    ...
    stub.setRead({ status: "ok", messages: [message("1_0", "hello")], nextOffset: "1_0", upToDate: true, closed: true });
  ...
  it("marks binary SSE responses with the base64 data encoding header", async () => {
    const stub = makeProtocolStub();
    stub.setMetadata({ status: "ok", contentType: "application/octet-stream", nextOffset: "0_0", closed: true });
...
describe("MetadataHttpService and DeleteHttpService", () => {
  it("maps metadata headers and delete statuses", async () => {
    const stub = makeProtocolStub();
    stub.setMetadata({ status: "ok", contentType: "text/plain", nextOffset: "1_0", ttlSeconds: 30, expiresAt: "2026-01-01T00:00:00Z", closed: true });
    const responses = new HttpResponseFactory();
    const metadata = await new MetadataHttpService({ protocol: stub.protocol, responses }).execute("s");
    ...
    const deleted = await new DeleteHttpService({ protocol: stub.protocol, responses }).execute("s");
    expect(deleted.status).toBe(204);
    stub.setDelete({ status: "gone" });
    expect((await new DeleteHttpService({ protocol: stub.protocol, responses }).execute("s")).status).toBe(410);
  });
});
...
describe("HttpDispatchService", () => {
  it("routes by method and applies security headers", async () => {
    ...
    const dispatch = new HttpDispatchService({
      path: new StreamPathService("/api"),
      responses,
      create: new CreateHttpService({ protocol: stub.protocol, path: new StreamPathService("/api"), responses, bodyReader: new RequestBodyReader(1024, responses) }),
      append: new AppendHttpService({ protocol: stub.protocol, responses, bodyReader: new RequestBodyReader(1024, responses), producerHeaders: new ProducerHeaderParser() }),
      read: new ReadHttpService({ protocol: stub.protocol, responses, bodyCodec, readQuery: new ReadQueryParser(), etags: new EtagBuilder(), longPoll, sse }),
    ...
    expect((await dispatch.fetch(new Request("http://x/api/s", { method: "PATCH" }))).status).toBe(405);
    expect((await dispatch.fetch(new Request("http://x/nope", { method: "GET" }))).status).toBe(400);
  });
  ...
  it("maps service exceptions to 500 responses", async () => {
    ...
    const dispatch = new HttpDispatchService({
      ...
      responses,
      create: new ThrowingCreateService({ protocol: stub.protocol, path: new StreamPathService("/"), responses, bodyReader: new RequestBodyReader(1024, responses) }),
      append: new AppendHttpService({ protocol: stub.protocol, responses, bodyReader: new RequestBodyReader(1024, responses), producerHeaders: new ProducerHeaderParser() }),
      read: new ReadHttpService({ protocol: stub.protocol, responses, bodyCodec, readQuery: new ReadQueryParser(), etags: new EtagBuilder(), longPoll, sse }),
      metadata: new MetadataHttpService({ protocol: stub.protocol, responses }),
      delete: new DeleteHttpService({ protocol: stub.protocol, responses }),
