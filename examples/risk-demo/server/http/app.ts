...
export interface AppDeps {
  ...
  /** `Hex Domination` defence window; injectable so tests need not wait 15s. */
...
export interface App {
  ...
  readonly boardRuntime: BoardRuntimeCache;
...
export function buildApp(deps: AppDeps): App {
  ...
  const boardCache = deps.boardCache ?? createBoardRuntimeCache(deps.protocol);
  ...
  return {
    boardRuntime: boardCache,
    defenseTimers,
    async close() {
      defenseTimers.stop();
      await boardCache.runtime.dispose();
      await boardCache.client.close();
    },
    async fetch(request) {
