...
import { RiskErrorCode } from "../../src/domain/commands.ts";
import { Schema } from "effect";

const CapabilityRoleSchema = Schema.Literals(["host", "player", "agent"]);
const GenerationStatusSchema = Schema.Literals(["building", "active", "retired", "failed"]);
const CommandStatusSchema = Schema.Literals(["accepted", "rejected"]);
const EventsJson = Schema.fromJsonString(Schema.Array(GameEvent));
const DecisionErrorJson = Schema.fromJsonString(
  Schema.Struct({
    code: RiskErrorCode,
    message: Schema.String,
    currentTurnId: Schema.optional(Schema.String),
    attackId: Schema.optional(Schema.String),
  }),
);

export function parseCapabilityRole(value: string): CapabilityRole {
  return Schema.decodeUnknownSync(CapabilityRoleSchema)(value);
}

export function parseGenerationStatus(value: string): GenerationStatus {
  return Schema.decodeUnknownSync(GenerationStatusSchema)(value);
}

export function parseCommandStatus(value: string): CommandRow["status"] {
  return Schema.decodeUnknownSync(CommandStatusSchema)(value);
}
