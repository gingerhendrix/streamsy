 ...
 *
 * Ordering follows the feed's own rule (`boardRowsFromQueries` sorts moves with
 * the same {@link compareMoves} total order), but the comparison is made here
 * rather than assumed, so the derivation is correct whichever way round the
...
/**
 * Total order over the moves feed.
 *
 * A source offset alone is not an order. A delivery boundary gives every event
 * in it the same position, so a command that emits several events puts several
 * moves at one offset — and "newest by offset" then picks whichever the scan
 * happened to reach first, which for a throw that resolves and captures is the
 * *earlier* half of the same command.
 *
 * `id` is the canonical event ordinal, zero-padded so codepoint order is
 * numeric order, which is exactly the tiebreak the offset cannot provide.
 */
export function compareMoves(
  left: { sourceOffset: string; id: string },
  right: { sourceOffset: string; id: string },
): number {
  const byOffset = compareOffsets(left.sourceOffset, right.sourceOffset);
  return byOffset === 0 ? compareOffsets(left.id, right.id) : byOffset;
}
...
export function latestAttackTrace(moves: readonly ProjectedMove[]): AttackTrace | null {
  ...
  for (const move of moves) {
    ...
    if (!newest || compareMoves(move, newest) > 0) newest = move;
  }
