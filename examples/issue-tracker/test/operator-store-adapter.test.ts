...
test("A2 patch, output rows, snapshot, history, and A4 cursor commit atomically", async () => {
  const store = memoryService(makeMemoryBacking());
  const result = maintainGraph({
    plan: projectBoard.plan,
    parameters: { projectId: "streamsy" },
    inputs: [
      { sourceId: "issue-tracker.issues", changes: [{ kind: "enter", key: "i1", after: issue }] },
      {
  ...
  await Effect.runPromise(store.commit(commit));
  expect(
    (await Effect.runPromise(store.snapshotRows({ ...commit.identity, id: projectBoard.name })))
      .rows,
  ).toHaveLength(1);
  expect(await Effect.runPromise(store.sourceProgress(commit.identity))).toBe("batch-1");
  const stored = await Effect.runPromise(
    store.getOperatorValue(
  ...
  const rejected = await Effect.runPromise(Effect.result(store.commit(stale)));
