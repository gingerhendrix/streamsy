...
const CursorBody = Schema.Struct({
...
describe("Integration 3B cross-object exchange", () => {
  test("registers a cold workspace and moves its assignment through the global alarm owner", async () => {
    ...
    const inbox = await eventually(async () => {
      const current = Schema.decodeUnknownSync(InboxBody)(
        await (await harness.fetch("/api/users/ada/inbox")).json(),
      );
      return current.rows.length === 1 ? current : undefined;
    });
    expect(inbox).toMatchObject({
      userId: "ada",
      rows: [{ workspaceId: "cold", issueId: "issue-cold", userId: "ada" }],
    });
    const sources = await (await harness.fetch("/api/global/sources")).json();
    expect(sources).toMatchObject({
      sources: [{ partition: "workspace:cold", kind: "workspace", id: "cold" }],
    });
  });
  ...
  test("source, user, and global eviction replay to one inbox row and one cursor advance", async () => {
    ...
    await harness.evictGlobal();
    await harness.runGlobalExchange();
    const inbox = Schema.decodeUnknownSync(InboxBody)(
      await (await harness.fetch("/api/users/ada/inbox")).json(),
    );
    expect(inbox.rows).toHaveLength(1);
    const state = Schema.decodeUnknownSync(CursorBody)(
      await (await harness.fetch("/api/global/exchange")).json(),
    );
    expect(state.cursors).toMatchObject([
      { source: { kind: "workspace", id: "main" }, applied: 1 },
    ]);
  });
  ...
  test("a new polling attempt observes facts appended after an earlier empty page", async () => {
    ...
    expect(
      (
        await harness.fetch(
          "/api/workspaces/late/issues",
          jsonRequest("POST", {
            commandId: "create-late",
            issueId: "issue-late",
            projectId: "streamsy",
            title: "late",
            status: "todo",
    ...
    expect(
      (
        await harness.fetch(
          "/api/workspaces/late/issues/issue-late/assignee",
          jsonRequest("POST", {
            commandId: "assign-late",
            assigneeId: "ada",
          }),
        )
      ).status,
