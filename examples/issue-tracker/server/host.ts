...
export class StreamHttp extends Context.Service<StreamHttp, { readonly fetch: (request: Request) => Promise<Response> }>()("IssueTracker/StreamHttp") {}

const streamHttpLayer = Layer.effect(StreamHttp, Effect.gen(function* () {
...
export const hostLayer = (filename: string) => ProjectionSqlite.layer.pipe(
  Layer.provideMerge(BunStorage.layerProtocol({ client: { filename } })),
);

export class ApplicationReady extends Context.Service<ApplicationReady, true>()("IssueTracker/ApplicationReady") {}
...
export const applicationLayer = (filename: string, workspaces: ReadonlyArray<string> = []) => {
  ...
  const ready = Layer.effect(ApplicationReady, Effect.gen(function* () {
    yield* prepareSchema;
    yield* createInputs(workspaces.flatMap(refs));
    if (workspaces.length > 0) yield* Projection.onChange(issueRows, workspaces.map((workspaceId) => ({ workspaceId })));
    return true as const;
  })).pipe(Layer.provide(base));
  return Layer.effectContext(Effect.context<Host | SqlClient.SqlClient | StreamHttp | ApplicationReady>()).pipe(
    Layer.provide(ready.pipe(Layer.provideMerge(base))),
  );
};

export const createInputs = (refs: ReadonlyArray<Parameters<typeof Streams.create>[0]>) =>
  Effect.forEach(refs, (ref) => Streams.create(ref).pipe(Effect.ignore), { discard: true });
