...
import { drainNotifications, listNotifications } from "../../application/application.ts";
...
import { handle } from "../../transport/router.ts";
import { applicationLayer } from "../../runtime.ts";
import { migratedSqlLayer } from "../../persistence/store-sql.ts";
import type { StreamGateway } from "../../transport/gateway.ts";
import {
  readAssignmentActivity,
  EXCHANGE_SOURCE_PAGE_LIMIT,
} from "../../exchange/exchange-source.ts";
import { InboxStore, migratedInboxSqlLayer } from "../../exchange/inbox-store.ts";
import { handleUserRequest, type UserServices } from "../../exchange/user-domain.ts";
import {
  handleGlobalRequest,
  migratedGlobalSqlLayer,
  type GlobalServices,
} from "../../exchange/global-domain.ts";
import { ExchangeCursorStore } from "../../exchange/exchange-store.ts";
...
export class WorkspacePartitionObject extends DurableObject<CloudflareEnv> {
  ...
  private async readExchangePage(
    request: Request,
    actor: ActorRuntime,
    id: string,
  ): Promise<Response> {
    ...
    const existing = (
      await Effect.runPromise(
        sql.unsafe<{ readonly request_hash: string; readonly response: string }>(
    ...
    if (existing !== undefined) {
      if (existing.request_hash !== requestHash)
        return json({ error: "operation-id-conflict" }, 409, id);
...
export class UserPartitionObject extends DurableObject<CloudflareEnv> {
  ...
  private initialize(canonicalKey: string) {
    ...
    this.runtime = this.ctx.blockConcurrencyWhile(async () => {
      const decoded = Schema.decodeUnknownSync(StoredUserPartition)(
        parsePartitionKey(canonicalKey),
      );
      if (partitionKeyString(userKey(decoded.id)) !== canonicalKey)
        throw new Error(`non-canonical user key ${canonicalKey}`);
      const stored = this.ctx.storage.kv.get<string>(PARTITION_HEADER);
      if (stored !== undefined && stored !== canonicalKey)
        throw new Error(`object is already bound to ${stored}`);
      if (stored === undefined) this.ctx.storage.kv.put(PARTITION_HEADER, canonicalKey);
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      const key =
        request.headers.get(PARTITION_HEADER) ?? this.ctx.storage.kv.get<string>(PARTITION_HEADER);
      ...
      if (url.pathname === "/_streamsy/exchange/apply-inbox") {
        ...
        if (input.destination.id !== actor.userId)
          return json({ error: "wrong-user-object" }, 409, id);
        const expectedHash = await stableHash({
          source: input.source,
          destination: input.destination,
          fromArrival: input.fromArrival,
          toArrival: input.toArrival,
          rows: input.rows,
        });
        if (expectedHash !== input.payloadHash)
          return json({ error: "payload-hash-mismatch" }, 409, id);
        const result = await actor.runtime.runPromise(
          Effect.gen(function* () {
            const inbox = yield* InboxStore;
            return yield* inbox.applyBatch(actor.userId, {
              operationId: input.operationId,
      ...
      if (
        resolution.kind !== "partition" ||
        resolution.key.kind !== "user" ||
        resolution.key.id !== actor.userId
      ) {
...
export class GlobalExchangeObject extends DurableObject<CloudflareEnv> {
  ...
  private initialize(canonicalKey: string) {
    if (this.runtime !== undefined) return this.runtime;
    this.runtime = this.ctx.blockConcurrencyWhile(async () => {
      if (canonicalKey !== partitionKeyString(globalKey()))
        throw new Error(`non-canonical global key ${canonicalKey}`);
      const stored = this.ctx.storage.kv.get<string>(PARTITION_HEADER);
      if (stored !== undefined && stored !== canonicalKey)
        throw new Error(`object is already bound to ${stored}`);
      if (stored === undefined) this.ctx.storage.kv.put(PARTITION_HEADER, canonicalKey);
      ...
      await Effect.runPromise(
        Effect.gen(function* () {
          yield* sql.unsafe<
            Record<string, never>
          >(`CREATE TABLE IF NOT EXISTS exchange_registration_receipts (
  ...
  private current() {
    return this.initialize(partitionKeyString(globalKey()));
  }
    ...
    try {
      ...
      if (url.pathname === "/_streamsy/exchange/register") {
        ...
        const result = await Effect.runPromise(
          Effect.gen(function* () {
            const sql = actor.sql;
            const receipt = (yield* sql.unsafe<{
              readonly request_hash: string;
              readonly response: string;
            }>(
              "SELECT request_hash, response FROM exchange_registration_receipts WHERE operation_id = ?",
              [input.operationId],
            ))[0];
            if (receipt !== undefined) {
              if (receipt.request_hash !== requestHash)
                return yield* Effect.fail("operation-id-conflict");
            ...
            yield* sql.withTransaction(
              Effect.gen(function* () {
                ...
                yield* sql.unsafe<Record<string, never>>(
                  "INSERT INTO exchange_schedule (source) VALUES (?) ON CONFLICT(source) DO NOTHING",
                  [partitionKeyString(input.source)],
                );
                yield* sql.unsafe<Record<string, never>>(
                  "INSERT INTO exchange_registration_receipts (operation_id, request_hash, response, created_at_ms) VALUES (?, ?, ?, ?)",
                  [input.operationId, requestHash, response, Date.now()],
                );
              }),
            );
            return response;
  ...
  async alarm(): Promise<void> {
    await this.serialized(() => this.runTurn());
  }
  ...
  private async runTurn(): Promise<void> {
    ...
    for (const source of sources) {
      try {
        immediate = (await this.exchangeSource(source.source)) || immediate;
      } catch (cause) {
        await this.recordFailure(source, cause);
      }
    }
  ...
  private async exchangeSource(sourceName: string): Promise<boolean> {
    ...
    const actor = await this.current();
    const attemptSequence = await Effect.runPromise(
      actor.sql.withTransaction(
        Effect.gen(function* () {
          const row = (yield* actor.sql.unsafe<{ readonly next_attempt_sequence: number }>(
            "SELECT next_attempt_sequence FROM exchange_schedule WHERE source = ?",
            [sourceName],
          ))[0];
    ...
    const cursor = await actor.runtime.runPromise(
      Effect.gen(function* () {
    ...
    const pageRequest: typeof ReadAssignmentPageRequest.Type = {
      operationId: `${attemptId}/page`,
      exchange: EXCHANGE_NAME,
      version: EXCHANGE_VERSION,
      source: parsed,
      afterArrival: cursor.arrival,
      limit: EXCHANGE_SOURCE_PAGE_LIMIT,
    };
      ...
      actor.sql.withTransaction(
        Effect.gen(function* () {
          const existing = (yield* actor.sql.unsafe<{ readonly request_hash: string }>(
            "SELECT request_hash FROM exchange_attempts WHERE attempt_id = ?",
            [attemptId],
          ))[0];
    ...
    const pageResponse = await this.env.WORKSPACES.get(
      this.env.WORKSPACES.idFromName(sourceName),
    ).fetch(
      new Request("http://workspace.internal/_streamsy/exchange/read-page", {
        method: "POST",
        headers: { "content-type": "application/json", [PARTITION_HEADER]: sourceName },
        body: JSON.stringify(pageRequest),
      }),
    );
    if (!pageResponse.ok)
      throw new Error(`source-read:${pageResponse.status}:${await pageResponse.text()}`);
    ...
    for (const record of page.records) {
      ...
      if ("_tag" in destination || destination.kind !== "user")
        throw new Error("source-poison:destination-key");
    ...
    for (const [userId, rows] of grouped) {
      const payload = {
        source: parsed,
        destination: userKey(userId),
        fromArrival: page.fromArrival,
        toArrival: page.toArrival,
        rows,
      };
      const input: typeof ApplyInboxBatchRequest.Type = {
        operationId: `${attemptId}/user/${userId}`,
        exchange: EXCHANGE_NAME,
        version: EXCHANGE_VERSION,
        ...payload,
        payloadHash: await stableHash(payload),
      };
      const response = await this.env.USERS.get(this.env.USERS.idFromName(`user:${userId}`)).fetch(
        new Request("http://user.internal/_streamsy/exchange/apply-inbox", {
          method: "POST",
          headers: { "content-type": "application/json", [PARTITION_HEADER]: `user:${userId}` },
    ...
    await actor.runtime.runPromise(
      Effect.gen(function* () {
        const sql = actor.sql;
          ...
          Effect.gen(function* () {
            ...
            if (current.arrival === cursor.arrival) {
              yield* cursors.advance({
                ...current,
                arrival: page.toArrival,
                applied: current.applied + applied,
              });
              yield* sql.unsafe<Record<string, never>>(
                "UPDATE exchange_sources SET last_exchanged_at_ms = ? WHERE source = ?",
                [Date.now(), sourceName],
              );
              yield* sql.unsafe<Record<string, never>>(
                "UPDATE exchange_schedule SET failure_count=0, next_eligible_at_ms=0, last_error=NULL WHERE source = ?",
                [sourceName],
              );
              yield* sql.unsafe<Record<string, never>>(
                "INSERT INTO exchange_attempts (attempt_id, source, expected_arrival, status, request_hash, applied, created_at_ms, completed_at_ms) VALUES (?, ?, ?, 'completed', ?, ?, ?, ?) ON CONFLICT(attempt_id) DO UPDATE SET status='completed', completed_at_ms=excluded.completed_at_ms",
                [
                  attemptId,
                  sourceName,
                  cursor.arrival,
  ...
  private async recordFailure(source: ScheduleSourceRow, cause: unknown): Promise<void> {
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        const sql = actor.sql;
        yield* sql.unsafe<Record<string, never>>(
          "UPDATE exchange_schedule SET failure_count=?, next_eligible_at_ms=?, last_error=? WHERE source=?",
          [count, Date.now() + delay, String(cause).slice(0, 1_000), source.source],
        );
        yield* sql.unsafe<Record<string, never>>(
          "UPDATE exchange_attempts SET status='failed' WHERE source=? AND status='allocated'",
          [source.source],
        );
      }),
    );
