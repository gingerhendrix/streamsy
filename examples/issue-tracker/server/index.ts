...
const databasePath = process.env.ISSUE_TRACKER_DB ?? `/tmp/streamsy-issue-tracker-${process.pid}.sqlite`;
...
const pathMatch = (pathname: string) => pathname.match(/^\/api\/workspaces\/([^/]+)\/(commands|issues|changes|drafts|status)$/);
...
const api = async (request: Request): Promise<Response> => {
  ...
  if (!workspaces.includes(workspaceId)) return json({ error: "Unknown workspace" }, { status: 404 });
  try {
    if (request.method === "POST" && resource === "commands") {
      ...
      await runtime.runPromise(Projection.serialized(issueRows.member({ workspaceId }), { limit: 5 }));
      const rows = await readIssues(workspaceId);
      return json({ commandId: command.commandId, event: accepted.event, row: rows.find((row) => row.issueId === command.issueId) });
    }
    if (request.method !== "GET") return json({ error: "Method not allowed" }, { status: 405 });
    if (resource === "issues") return json({ workspaceId, rows: await readIssues(workspaceId) });
    if (resource === "changes") return json({ workspaceId, rows: await query(`SELECT * FROM issue_changes WHERE workspace_id=? ORDER BY id`, [workspaceId]) });
    if (resource === "drafts") return json({ workspaceId, rows: await query(`SELECT * FROM notification_drafts WHERE workspace_id=? ORDER BY event_id`, [workspaceId]) });
    ...
    return json({ workspaceId, offsets: Option.match(loaded.record, { onNone: () => ({}), onSome: (record) => record.inputs }) });
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : String(error) }, { status: 400 });
  }
};

const query = <A extends object>(statement: string, params: ReadonlyArray<unknown>) => runtime.runPromise(
  Effect.gen(function* () { const sql = yield* SqlClient.SqlClient; return yield* sql.unsafe<A>(statement, params); }),
);
const readIssues = async (workspaceId: string) => {
  const rows = await query<Record<string, unknown> & { issueId: string }>(`SELECT i.issue_id AS issueId,i.project_id AS projectId,i.title,i.status,i.sequence,i.updated_at AS updatedAt,i.assignee_id AS assigneeId,COALESCE(GROUP_CONCAT(CASE WHEN il.attached=1 THEN il.label_id END), '') AS labelIds FROM issues i LEFT JOIN issue_labels il ON il.workspace_id=i.workspace_id AND il.issue_id=i.issue_id WHERE i.workspace_id=? GROUP BY i.issue_id ORDER BY i.issue_id`, [workspaceId]);
  return rows.map((row) => ({ ...row, assigneeId: row.assigneeId ?? undefined, labelIds: String(row.labelIds).split(",").filter(Boolean) }));
};
...
export const shutdown = () => closing ??= (async () => { await server.stop(true); await runtime.dispose(); })();
