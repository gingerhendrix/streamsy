...
/** Routes only ever hand the builders a body that already parsed. */
function parsed<T>(schema: { parse: (value: unknown) => T }, body: unknown): T {
  return schema.parse(body);
}

describe("newProject", () => {
  test("fills server defaults for absent fields", () => {
    const project = newProject(parsed(projectInput, {}));
  ...
  test("trims the supplied name and description", () => {
    const project = newProject(parsed(projectInput, { name: "  Streamsy  ", description: " hi " }));
...
describe("newIssue", () => {
  test("defaults status to open and updatedAt to createdAt", () => {
    const issue = newIssue(parsed(issueInput, { projectId: "proj_1", title: " Ship " }));
  ...
  test("keeps a supplied status", () => {
    expect(newIssue(parsed(issueInput, { status: "done" })).status).toBe("done");
  });
...
describe("nextIssue", () => {
  const previous = newIssue(parsed(issueInput, { projectId: "proj_1", title: "Ship" }));

  test("keeps the previous title and status when the patch omits them", () => {
    const next = nextIssue(previous, parsed(issueInput, {}));
  ...
  test("applies a trimmed title and a new status", () => {
    const next = nextIssue(previous, parsed(issueInput, { title: " Shipped ", status: "done" }));
...
describe("newComment", () => {
  test("falls back to the default author for a blank one", () => {
    const comment = newComment(parsed(commentInput, { author: "   ", body: " hi " }));
