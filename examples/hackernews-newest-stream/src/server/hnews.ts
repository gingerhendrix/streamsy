...
export async function fetchNewestStoryIds(limit: number): Promise<number[]> {
  ...
  if (!response.ok) throw new Error(`HN newstories failed: ${response.status} ${response.statusText}`);
...
export async function fetchStory(id: number, rank: number): Promise<HnStory | null> {
  ...
  if (!item || item.deleted || item.dead || item.type !== "story" || !item.title || !item.time) return null;
