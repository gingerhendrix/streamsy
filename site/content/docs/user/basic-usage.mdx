...
1. a **`StreamFactory`** from a storage package (where messages are persisted);
2. a **protocol facade** from `createStreamProtocol({ storage: { factory } })` (the Durable Streams operations);
3. an **HTTP facade** from `createHttpHandler({ protocol, pathPrefix })` (serves the protocol over HTTP).

The protocol and HTTP facades never change — only the factory you pass decides where data lives.

## Install

Streamsy targets the Bun runtime. Start with the core package, which bundles the in-memory factory:
...
The server now speaks the Durable Streams HTTP protocol on the configured `pathPrefix`. The in-memory factory keeps everything in process — ideal for development, examples, and conformance testing, but not durable across restarts.
...
To persist data, swap `createMemoryStreamFactory()` for a durable factory. Nothing else in the composition changes.
...
const factory = createSqliteStreamFactory({ filename: "streams.db" }); // omit for :memory:
const protocol = createStreamProtocol({ storage: { factory } });
const handler = createHttpHandler({ protocol, pathPrefix: "/" });

Bun.serve({ fetch: (request) => handler.fetch(request) });

// factory.close() closes the underlying database connection.
```
...
export default {
  fetch(request: Request, env: Env) {
    const factory = createDurableObjectStreamFactory({ namespace: env.STREAMS });
    const protocol = createStreamProtocol({ storage: { factory } });
    const handler = createHttpHandler({ protocol, pathPrefix: "/" });
...
Bind `STREAMS` to the `DurableObjectStreamStorage` class in your Wrangler config (with a SQLite-backed migration). The factory routes each stream id to its Durable Object via `namespace.idFromName(streamId)`.
