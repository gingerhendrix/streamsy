...
`DurableStateStream<S>` also exposes `readNext(options)`, `metadata()`, `delete()`, and the underlying `json` / `stream` handles for byte- or JSON-level access. Application code can fold these messages into its own collections. Streamsy does not currently ship a generic TanStack DB adapter in `@streamsy/state`.
