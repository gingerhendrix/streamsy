...
semantics. Forks require the source and child to share an object. The default
`byStream()` placement does not support forks and answers 404 for a source in
another object; use `byKey` to co-locate each fork family. These local results
do not establish hosted behavior.
...
`{ pathPrefix }` through its HTTP options. A prefix is a route boundary, not an authorization boundary. Authenticate
