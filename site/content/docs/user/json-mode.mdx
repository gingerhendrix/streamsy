---
title: JSON mode
description: Typed JSON streams with @streamsy/core/json — JsonProtocol, JsonStream, codecs, and Standard Schema.
---

`@streamsy/core/json` wraps a `StreamProtocolAdapter` so messages are typed JavaScript values instead of raw bytes. Values are encoded to / decoded from `application/json` through a **codec** or a **Standard Schema**, and streams are tagged with the `application/json` content type.
...
- `read(options?)` / `readNext(options)` — decode stored messages into `{ ...message, value: T }`.
...
- **`read` / `readNext`** → `ok` with decoded `messages`, or `invalid-json` (`{ error, offset? }`) when a stored value fails the codec/schema, plus the underlying `timeout` / `not-found` / `gone` statuses.
